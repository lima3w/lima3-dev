...
description = "A homelab cleanup focused on DNS high availability, repeatable VM builds, and audit-mode configuration management."
authors = ["Zack"]
categories = ["Homelab"]
...
## Making DNS less fragile

DNS was the big one. I have been running [Technitium DNS](https://technitium.com/dns/) as DNS and DHCP for the network, but it was all on one system. DNS is one of those services that nobody thinks about until it stops working, then nothing else seems to work either. I set up a second Technitium instance on a separate VM and started working on clustering them together.
...
## Taking the randomness out of VM builds
